...
export interface OAuthEnvironment {
  ...
  readonly ANILIST_CLIENT_SECRET: RuntimeSecret;
  readonly MAL_CLIENT_ID: string;
  readonly MAL_CLIENT_SECRET: RuntimeSecret;
}
...
export const getOAuthClientConfig = async (
  ...
  if (provider === "anilist") {
    return {
      ...
      clientSecret: await readSecret(env.ANILIST_CLIENT_SECRET, "ANILIST_CLIENT_SECRET"),
