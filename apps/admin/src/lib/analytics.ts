...
async function fetchSnapshot(): Promise<AnalyticsSnapshot> {
  ...
  try {
    ...
    const apiToken = await resolveSecret(env.ADMIN_PANEL_ANALYTICS_API);
    ...
    if (
      ...
      return emptyAnalyticsSnapshot(
        "ADMIN_PANEL_ANALYTICS_API / CF_ACCOUNT_ID bindings are not configured",
        fetchedAt,
      );
    }
...
async function fetchCacheSnapshot(): Promise<CacheSnapshot> {
  ...
  try {
    ...
    if (!isStringValue(apiToken) || apiToken === "") {
      return emptyCacheSnapshot("ADMIN_PANEL_ANALYTICS_API binding is not configured", fetchedAt);
    }
