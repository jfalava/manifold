...
export const md2alCommand = Command.make(
  "md2al",
  {
    anilistToken: Flag.string("anilist-token").pipe(
      Flag.optional,
      Flag.withDescription("AniList access token. Falls back to ANILIST_TOKEN."),
    ),
    mangadexToken: Flag.string("mangadex-token").pipe(
      Flag.optional,
      Flag.withDescription(
        "MangaDex personal token with manga.read scope. Falls back to MANGADEX_TOKEN, or to the password-grant credentials below.",
      ),
    ),
    ...
    mangadexClientId: Flag.string("mangadex-client-id").pipe(
      Flag.optional,
      Flag.withDescription("Falls back to MANGADEX_CLIENT_ID."),
    ),
    mangadexClientSecret: Flag.string("mangadex-client-secret").pipe(
      Flag.optional,
      Flag.withDescription("Falls back to MANGADEX_CLIENT_SECRET."),
    ),
    mangadexUsername: Flag.string("mangadex-username").pipe(
      Flag.optional,
      Flag.withDescription("Falls back to MANGADEX_USERNAME."),
    ),
    mangadexPassword: Flag.string("mangadex-password").pipe(
      Flag.optional,
      Flag.withDescription("Falls back to MANGADEX_PASSWORD."),
    ),
  },
  ({
    anilistToken,
    ...
    Effect.gen(function* () {
      ...
      // iac/.env convention: secrets live under ALCHEMY_SECRET_ prefixed
      // names, so check both spellings.
      const secretEnv = (base: string): string =>
        env(base) || env(`ALCHEMY_SECRET_${base}`);

      const anilist = flag(anilistToken) || secretEnv("ANILIST_TOKEN");
      const credentials = {
        clientId: flag(mangadexClientId) || secretEnv("MANGADEX_CLIENT_ID"),
        clientSecret:
          flag(mangadexClientSecret) || secretEnv("MANGADEX_CLIENT_SECRET"),
        username: flag(mangadexUsername) || secretEnv("MANGADEX_USERNAME"),
        password: flag(mangadexPassword) || secretEnv("MANGADEX_PASSWORD"),
      };
      ...
      let mdToken = flag(mangadexToken) || env("MANGADEX_TOKEN");
      ...
      if (!mdToken) {
        return yield* Effect.fail(
          new Error(
            "Missing MangaDex token: pass --mangadex-token, set MANGADEX_TOKEN, or provide the password-grant flags.",
          ),
        );
      }
