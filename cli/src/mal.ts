...
export const createMalAuthorization = (clientId: string) => {
  ...
  url.search = new URLSearchParams({
    response_type: "code", client_id: clientId, redirect_uri: MAL_REDIRECT_URI,
    state, code_challenge: verifier, code_challenge_method: "plain",
  }).toString();
...
export const createMalClient = (options: {
  ...
  const request = async (path: string, method = "GET", body?: URLSearchParams): Promise<Response> => {
    ...
    for (let attempt = 0; ; attempt += 1) {
      ...
      if ((response.status === 429 || response.status >= 500) && attempt < 3) {
        ...
        const wait = retryAfter === null ? NaN : Number.isFinite(seconds)
          ? seconds * 1000 : Date.parse(retryAfter) - Date.now();
        await response.body?.cancel();
        if (wait > 300_000) {throw new Error("MAL requested a long retry delay. Stop and resume later.");}
        await sleep(Number.isFinite(wait) ? Math.max(1500, wait) : 5000 * 2 ** attempt);
        continue;
      }
      if (!response.ok && !(method === "DELETE" && response.status === 404)) {
        await response.body?.cancel();
        throw new Error(`MAL ${method} ${path}: HTTP ${response.status}. Stopped; rerun to resume.`);
      }
      return response;
    }
  };
  ...
  return {
    profile: async () => Schema.decodeUnknownSync(Profile)(await (await request("/users/@me")).json()),
    manga: async (): Promise<MalManga[]> => {
      ...
      while (path) {
        if (visited.has(path)) {throw new Error("MAL pagination repeated a page; refusing an incomplete scan.");}
        visited.add(path);
        const page = Schema.decodeUnknownSync(MangaPage)(await (await request(path)).json());
        for (const { node } of page.data) {
          if (node.id <= 0) {throw new Error("MAL returned an invalid manga ID.");}
          entries.set(node.id, node);
        }
        path = undefined;
        if (page.paging.next) {
          const next = new URL(page.paging.next);
          if (next.origin !== "https://api.myanimelist.net" || next.pathname !== "/v2/users/@me/mangalist") {
    ...
    deleteManga: async (id: number): Promise<void> => {
      if (!Number.isSafeInteger(id) || id <= 0) {throw new Error("Invalid MAL manga ID.");}
    ...
    updateManga: async (id: number, values: MalMangaUpdate): Promise<void> => {
      ...
      if (body.size === 0) {throw new Error("MAL manga update needs at least one field.");}
...
export const wipeMalManga = async (options: {
  ...
  const summary = { account: profile.name, accountId: profile.id, scanned: entries.length, deleted: 0 };
  ...
  if ((await options.client.profile()).id !== profile.id) {throw new Error("MAL account changed; refusing deletion.");}
  ...
  if (remaining.length) {
    throw new Error(`MAL wipe incomplete: ${remaining.length} manga remain (${remaining.map((entry) => entry.id).join(", ")}). Check other writers and rerun.`);
  }
  return summary;
};
