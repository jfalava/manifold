...
import { ANILIST_REDIRECT_URI, ANILIST_SECRET, createAniListAuthorization, exchangeAniListCode, validateAniListSession } from "@/anilist-auth";
...
const login = Command.make("login", {
  ...
  Command.withDescription(`Authorize AniList locally. Register ${ANILIST_REDIRECT_URI} on a separate authorization-code client.`),
  Command.withHandler(({ clientId }) => Effect.tryPromise({
    try: async () => {
      ...
      if (!id || !secret) {throw new Error("Set MANIFOLD_ANILIST_CLIENT_ID and MANIFOLD_ANILIST_CLIENT_SECRET for the CLI OAuth application.");}
      openFrame("anilist login");
      const auth = createAniListAuthorization(id);
      const callback = Promise.withResolvers<string>();
      const server = Bun.serve({
        hostname: "127.0.0.1", port: 8767,
        fetch(request) {
          const url = new URL(request.url);
          const headers = { "content-type": "text/plain", "cache-control": "no-store" };
          if (request.method !== "GET" || url.pathname !== "/callback") {return new Response("Not found", { status: 404, headers });}
          if (url.searchParams.get("state") !== auth.state) {return new Response("Invalid OAuth state", { status: 400, headers });}
      ...
      const timer = setTimeout(() => callback.reject(new Error("AniList login timed out after five minutes.")), 300_000);
      try {
        ...
        if (process.env.MANIFOLD_ANILIST_TOKEN) {
          frameDetail("MANIFOLD_ANILIST_TOKEN is set and overrides this login. Unset it to use the keychain token.");
        }
      } finally {
        clearTimeout(timer);
...
export const anilistCommand = Command.make("anilist").pipe(
  Command.withDescription("AniList authentication."),
