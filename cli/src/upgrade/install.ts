...
const isRetryableNetworkError = (cause: unknown): boolean =>
  cause instanceof Error && /connection|fetch|network|socket|timeout/i.test(cause.message);
...
const downloadAttempt = async (
  ...
  try {
    const response = await fetcher(url, {
      headers: { "User-Agent": "manifold-cli" },
      redirect: "follow",
...
export const checksumFromFile = (contents: string): string => {
...
export const installRelease = async (release: CliRelease, targetPath: string): Promise<void> => {
  ...
  // Ad-hoc sign on macOS so Bun.secrets / keychain access is not killed (exit 137).
  if (process.platform === "darwin") {
    try {
      ...
      console.warn(
        `Warning: Could not codesign ${targetPath}: ${errorMessage(cause)}. The binary may be killed on keychain access.`,
      );
    }
  }
};
