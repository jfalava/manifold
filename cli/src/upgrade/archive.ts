...
const ensureRange = (offset: number, size: number, length: number, message: string): void => {
  if (offset < 0 || size < 0 || offset + size > length) {
    throw new Error(message);
  }
};

const findDirectory = (archive: Uint8Array, view: DataView): ZipDirectory => {
...
const readEntry = (archive: Uint8Array, view: DataView, offset: number): ZipEntry => {
...
const extractEntry = (archive: Uint8Array, view: DataView, entry: ZipEntry): Uint8Array => {
  ensureRange(
...
export const extractZipBinary = (archive: Uint8Array, expectedName: string): Uint8Array => {
