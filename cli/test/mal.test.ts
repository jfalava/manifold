...
import { createMalAuthorization, createMalClient, MAL_REDIRECT_URI, requestMalTokens, wipeMalManga } from "../src/mal";

afterEach(() => { vi.unstubAllGlobals(); vi.restoreAllMocks(); });

const profile = () => Response.json({ id: 7, name: "reader" });
const page = (ids: number[], next?: string) => Response.json({
...
describe("MAL manga wipe", () => {
  it("scans all pages before any delete, deduplicates, includes adult entries, and verifies empty", async () => {
    ...
    const ctx = setup(profile(), page([1, 2], next), page([2, 3]), profile(),
      new Response(null), new Response(null, { status: 404 }), new Response(null), page([]));
    ...
    expect(ctx.scanned).toHaveBeenCalledWith("reader", [
      { id: 1, title: "Manga 1" }, { id: 2, title: "Manga 2" }, { id: 3, title: "Manga 3" },
    ]);
    const calls = vi.mocked(fetch).mock.calls;
    expect(calls.map((call) => call[1]?.method)).toEqual(["GET", "GET", "GET", "GET", "DELETE", "DELETE", "DELETE", "GET"]);
    expect(calls.map((call) => String(call[0])).filter((url) => url.includes("mangalist")))
      .toSatisfy((urls: string[]) => urls.every((url) => new URL(url).searchParams.get("nsfw") === "true"));
    expect(calls.filter((call) => call[1]?.method === "DELETE").map((call) => String(call[0])))
  ...
  it("stops on failure and a new run deletes only surviving entries", async () => {
    const ctx = setup(profile(), page([1, 2]), profile(), new Response(null), new Response(null, { status: 403 }),
      profile(), page([2]), profile(), new Response(null), page([]));
    ...
    expect(deletes.map((call) => String(call[0]))).toEqual([1, 2, 2].map((id) => `https://api.myanimelist.net/v2/manga/${id}/my_list_status`));
  });
  ...
  it("rejects pagination loops", async () => {
    const ctx = setup(page([1], "https://api.myanimelist.net/v2/users/@me/mangalist?limit=1000&nsfw=true"));
...
describe("MAL manga updates", () => {
  it("PATCHes only supplied fields and preserves zero, false, and escaped text", async () => {
    ...
    await ctx.client.updateManga(1, {
      status: "reading", score: 0, is_rereading: false,
      num_chapters_read: 12, comments: "A&B = 漫画", tags: undefined,
    });
    ...
    expect(Object.fromEntries(new URLSearchParams(String(call?.[1]?.body)))).toEqual({
      status: "reading", score: "0", is_rereading: "false", num_chapters_read: "12", comments: "A&B = 漫画",
    });
  });

  it("rejects invalid IDs and empty updates without making requests", async () => {
    const ctx = setup();
    await expect(ctx.client.updateManga(0, { status: "reading" })).rejects.toThrow("Invalid MAL manga ID");
...
describe("MAL authentication and retries", () => {
  ...
  it("refreshes an expired session and persists rotated tokens before use", async () => {
    const ctx = setup(Response.json({ access_token: "new", refresh_token: "rotated", expires_in: 3600 }), profile());
    ...
    const client = createMalClient({
      ...
      clientSecret: "secret", saveSession, sleep: ctx.sleep,
    });
    await client.profile();
    expect(saveSession).toHaveBeenCalledWith(expect.objectContaining({ accessToken: "new", refreshToken: "rotated" }));
    const calls = vi.mocked(fetch).mock.calls;
  ...
  it("refreshes once on 401, then stops on another 401", async () => {
    const ctx = setup(new Response(null, { status: 401 }),
      Response.json({ access_token: "new", expires_in: 3600 }), new Response(null, { status: 401 }));
    const saveSession = vi.fn<() => Promise<void>>().mockResolvedValue(undefined);
    const client = createMalClient({
      session: { clientId: "client", accessToken: "old", refreshToken: "refresh", expiresAt: Date.now() + 3600_000 },
      saveSession, sleep: ctx.sleep,
    });
  ...
  it("honors Retry-After and bounds 5xx retries", async () => {
    const ctx = setup(new Response(null, { status: 429, headers: { "retry-after": "12" } }), profile(),
