...
search requires `MANIFOLD_MAL_CLIENT_ID`. Provider failures are returned per provider so
...
and secret into the four `MANIFOLD_MANGADEX_*` variables in `iac/.env`. MangaDex has no
...
without `MANIFOLD_TOKEN`; `MANIFOLD_OAUTH_TOKEN_ENCRYPTION_SECRET` is used to
