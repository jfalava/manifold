...
describe("Worker-side MAL backup outbox", () => {
  ...
  const request = async (input: JsonObject): Promise<JsonValue> => {
    ...
    if (!isJsonValue(body)) {throw new Error("Invalid fixture response");}
    return body;
  };
  ...
  const readOps = async () => Schema.decodeUnknownSync(Schema.Array(SyncOp))(
  ...
  beforeAll(async () => {
    ...
    worker = new Miniflare(convertV4MiniflareOptions({
      name: "mal-test", modules: true, script: built.outputFiles[0].text,
      compatibilityDate: "2026-08-28",
      durableObjects: { MANIFOLD_SYNC: { className: "TestSync", useSQLite: true } },
      bindings: { MANIFOLD_OAUTH_TOKEN_ENCRYPTION_SECRET: "test-encryption-key", MANIFOLD_MAL_CLIENT_ID: "test-client" },
      outboundService: async (outbound) => {
        const url = new URL(outbound.url);
        if (url.hostname !== "api.myanimelist.net") {throw new Error(`Unexpected upstream: ${url.hostname}`);}
        ...
        return WorkerResponse.json({ data: [{ node: { id: 900, title: "Title", alternative_titles: { synonyms: ["Alias"] } } }] });
      },
    }));
  ...
  it("backs up the latest tracker state, persists an ID binding, and retains canonical identity", async () => {
    ...
    await request({ action: "set", id: entry.id, change: {
      origin: "device", appliedRemotely: true, status: "reading",
      backupIdentity: { anilistId: "42", malId: "7", titles: ["Title", "Alias"] },
    } });
    await request({ action: "set", id: entry.id, change: { origin: "device", appliedRemotely: true, status: "completed" } });
    expect(writes).toHaveLength(0);
    expect((await readOps()).filter((op) => op.state === "pending")).toHaveLength(1);
    await request({ action: "drain" });
    expect(writes).toEqual([{ path: "/v2/manga/7/my_list_status", body: "status=completed&is_rereading=false" }]);
    const stored = Schema.decodeUnknownSync(RegistryEntry)(await request({ action: "get", id: entry.id }));
    expect(stored).toMatchObject({ id: entry.id, provider: "anilist", providerId: "42", title: "Title" });
    expect(stored.providers).toContainEqual(expect.objectContaining({ provider: "mal", externalId: "7" }));
  });

  it("finds an unbound entry by title and updates it in the same drain", async () => {
    const entry = await createEntry("43");
    await request({ action: "set", id: entry.id, change: { origin: "device", status: "on_hold", appliedRemotely: true } });
    await request({ action: "drain" });
    expect(writes.at(-1)).toEqual({ path: "/v2/manga/900/my_list_status", body: "status=on_hold&is_rereading=false" });
  });
  ...
  it("does not steal a MAL binding already owned by another entry", async () => {
    ...
    await request({ action: "set", id: entry.id, change: {
      origin: "device", status: "reading", appliedRemotely: true,
      backupIdentity: { anilistId: "44", malId: "7", titles: [] },
    } });
    await request({ action: "drain" });
    expect(writes).toHaveLength(before);
    expect((await readOps()).find((op) => op.payload.entryId === entry.id)).toMatchObject({
      state: "pending", attempts: 1, lastError: expect.stringContaining("another registry entry"),
    });
  ...
  it("keeps upstream failures out of the tracker response and retries current state", async () => {
    ...
    upstreamStatus = 503;
    const state = Schema.decodeUnknownSync(ListState)(await request({ action: "set", id: entry.id, change: {
      origin: "device", status: "reading", appliedRemotely: true,
    } }));
    expect(state.status).toBe("reading");
    await request({ action: "drain" });
    const failed = (await readOps()).find((op) => op.payload.entryId === entry.id);
    expect(failed).toMatchObject({ state: "pending", attempts: 1, lastError: expect.stringContaining("503") });
    upstreamStatus = 200;
    await request({ action: "set", id: entry.id, change: { origin: "device", status: "dropped", appliedRemotely: true } });
    await request({ action: "drain" });
  ...
  it("skips deleted entries and explicit status clears; admin updates do not originate backups", async () => {
    ...
    for (const entry of [deleted, cleared]) {
      await request({ action: "set", id: entry.id, change: { origin: "device", status: "reading", appliedRemotely: true } });
    }
    await request({ action: "nuke", id: deleted.id });
    await request({ action: "set", id: cleared.id, change: { origin: "device", status: null, appliedRemotely: true } });
  ...
  it("blocks repeated failures and supports explicit retry without replaying old state", async () => {
    ...
    await request({ action: "set", id: entry.id, change: { origin: "admin", status: "completed", appliedRemotely: true } });
    upstreamStatus = 200;
