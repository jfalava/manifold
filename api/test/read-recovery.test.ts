...
describe("queued reads with stale registry IDs", () => {
  ...
  const read = (provider: string, sourceMangaId: string, eventId: string) => ({
    eventId, provider, sourceMangaId, sourceChapterId: "chapter-12",
    chapterKey: `${provider}:chapter-12`, chapterNumber: 12, readAt: 1_788_800_000_000,
  });
  ...
  beforeAll(async () => {
    ...
    worker = new Miniflare(convertV4MiniflareOptions({
      name: "read-recovery-test", modules: true, script: built.outputFiles[0].text,
  ...
  it.each(["mangadex", "comix"])("recovers missing UUIDs using exact %s links and keeps retries idempotent", async (provider) => {
    const current = await entry(provider, `native-${provider}`);
    const input = { action: "read", id: `missing-${provider}`, read: read(provider, `native-${provider}`, `event-${provider}`) };
    ...
    if (provider === "mangadex") {
      const ops = Schema.decodeUnknownSync(Schema.Array(SyncOp))(await request({ action: "ops" }));
  ...
  it("keeps unresolved reads failed rather than inventing an entry or stealing a cross-provider ID", async () => {
    ...
    const input = { action: "read", id: "missing-unresolved", read: read("comix", "md-only", "unresolved-event") };
    expect(await request(input, 500)).toEqual({ error: "Canonical entry not found: missing-unresolved" });
    expect(await request({ action: "get", id: input.id })).toBeNull();
  });
  ...
  it("does not redirect an existing live entry with a conflicting native binding", async () => {
    ...
    expect(await request({ action: "read", id: first.id, read: read("comix", "second", "conflict-event") }, 500))
  ...
  it("still follows tombstoned entries to their live successors", async () => {
    ...
    expect(await request({ action: "read", id: old.id, read: read("comix", "recreated", "recreated-event") }))
