...
const candidate = (id: string, title: string, aliases: readonly string[] = []): CanonicalSearchResult => ({
  id: `mal:${id}`, provider: "mal", providerId: id, title, aliases, score: 0,
});
const entry: RegistryEntry = {
  id: "registry-id", provider: "anilist", providerId: "42", title: "English title",
...
const environment = (): Pick<Env, "AI" | "MANIFOLD_MAL_CLIENT_ID"> => {
  const fixture = {
    ...
    AI: { run: async () => { throw new Error("Unexpected embedding request"); } },
  };
...
describe("MAL backup matching", () => {
  it("matches any normalized title or alias, not only the display title", () => {
    expect(chooseMalMatch(["English title", "日本語"], [
  ...
  it("uses the MangaDex confidence and runner-up margin thresholds", () => {
    ...
    expect(chooseMalMatch(["Query"], ranked(0.92, 0.8))).toMatchObject({ externalId: "1", method: "semantic" });
  ...
  it("reuses an explicit binding ahead of supplied metadata without searching", async () => {
    expect(await resolveMalBackup(environment(), {
      ...entry, providers: [...entry.providers, { provider: "mal", externalId: "7", updatedAt: 0 }],
    }, { anilistId: "42", malId: "8", titles: [] })).toMatchObject({ externalId: "7", method: "binding" });
    expect(await resolveMalBackup(environment(), entry, {
  ...
  it("searches all distinct aliases and deduplicates returned MAL identities", async () => {
    ...
    const source: CanonicalSearchSource = {
      provider: "mal", getById: () => Effect.succeed(undefined),
      search: (term) => {
        terms.push(term);
    ...
    expect(await resolveMalBackup(environment(), entry, {
      anilistId: "42", titles: ["English title", "Romaji", "日本語", "ROMAJI"],
    }, source)).toMatchObject({ externalId: "7", method: "title" });
  ...
  it("ignores identity metadata from another AniList entry", async () => {
    ...
    const source: CanonicalSearchSource = {
      provider: "mal", getById: () => Effect.succeed(undefined),
      search: (term) => { terms.push(term); return Effect.succeed([]); },
    };
    await expect(resolveMalBackup(environment(), entry, {
      anilistId: "wrong", malId: "8", titles: ["Wrong title"],
    }, source)).rejects.toThrow("unmatched");
  ...
  it("never accepts a partial search after an alias request fails", async () => {
    const source: CanonicalSearchSource = {
      provider: "mal", getById: () => Effect.succeed(undefined),
      search: (term) => term === "English title"
        ? Effect.succeed([candidate("7", "English title")])
        : Effect.fail({ _tag: "CanonicalSourceError", provider: "mal", message: "rate limited", status: 429 }),
    };
    await expect(resolveMalBackup(environment(), entry, {
      anilistId: "42", titles: ["Alias"],
    }, source)).rejects.toBeDefined();
  });
});

describe("MAL backup status write", () => {
  const statuses: readonly ListStatus[] = ["reading", "completed", "on_hold", "dropped", "plan_to_read", "re_reading"];
  it.each(statuses)("projects %s without overwriting other list fields", async (status) => {
    ...
    expect(fetcher).toHaveBeenCalledWith("https://api.myanimelist.net/v2/manga/7/my_list_status", expect.objectContaining({
      method: "PATCH",
