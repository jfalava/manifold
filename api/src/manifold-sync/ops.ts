...
export async function listPendingSync(host: SyncHost): Promise<readonly SyncOp[]> {
  return Effect.runSync(Effect.sync(() => readOps(host, "mangadex", "pending")));
}

export async function retryFailedSync(host: SyncHost): Promise<{ retried: number }> {
  const timestamp = now();
...
export async function pendingAniListOps(host: SyncHost, limit = 25): Promise<readonly SyncOp[]> {
  return Effect.runSync(
    Effect.sync(() => readOps(host, "anilist", "pending", Math.min(100, Math.max(1, limit)))),
  );
}

export async function completeOps(
  host: SyncHost,
  input: CompleteOpsInput,
...
export async function retryOp(host: SyncHost, opId: string): Promise<SyncOp | undefined> {
  const timestamp = now();
  host.ctx.storage.sql.exec(
...
export async function opsSummary(host: SyncHost, limit = 200): Promise<OpsSummary> {
  return Effect.runSync(
    Effect.sync(() => {
...
export async function backfillMangaDexShelf(host: SyncHost): Promise<{ enqueued: number }> {
  const enqueued =
