...
export async function backupRegistry(host: SyncHost): Promise<RegistryBackupMetadata> {
  const bucket = host.env.REGISTRY_BACKUPS;
  if (!bucket) {
    throw new Error("Registry backup bucket is not configured");
  }
  const backup = await snapshotRegistry(host);
...
export async function listBackups(host: SyncHost): Promise<readonly RegistryBackupMetadata[]> {
  const bucket = host.env.REGISTRY_BACKUPS;
  if (!bucket) {
    throw new Error("Registry backup bucket is not configured");
  }
...
export async function restoreBackup(host: SyncHost, key: string): Promise<RegistryBackupMetadata> {
...
export async function snapshotRegistry(host: SyncHost): Promise<RegistryBackup> {
  ...
  return host.ctx.blockConcurrencyWhile(async () => {
    const createdAt = now();
