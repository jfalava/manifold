...
export async function scheduleSync(host: SyncHost, delayMs = 0): Promise<void> {
  ...
  const currentAlarm = await host.ctx.storage.getAlarm();
  if (currentAlarm === null || currentAlarm <= now() || currentAlarm > scheduledAt) {
    await host.ctx.storage.setAlarm(scheduledAt);
  }
}
